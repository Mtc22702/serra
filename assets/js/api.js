...
(() => {
  ...
  const SerraAPI = {
    ...
    async getPlants() {
      ...
      if (serverActive) {
        try {
          ...
          console.warn("Rilevato server ma errore nel caricamento piante, provo fallback...", e);
        }
      }
      ...
      try {
        const res = await fetchWithTimeout('./db/plants.json', { timeout: 1500 });
        if (res.ok) {
    ...
    async savePlants(plants) {
      ...
      if (serverActive) {
        try {
          ...
          console.error("Errore nel salvataggio sul server locale, salvo in localStorage...", e);
        }
      }
      // Salvataggio locale
      localStorage.setItem('serra.custom_plants', JSON.stringify(plants));
    ...
    async getUsers() {
      ...
      try {
        const res = await fetchWithTimeout('./db/users.json', { timeout: 1500 });
        if (res.ok) {
    ...
    async getOrders() {
      ...
      try {
        const res = await fetchWithTimeout('./db/orders.json', { timeout: 1500 });
        if (res.ok) {
