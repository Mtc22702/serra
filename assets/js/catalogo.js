...
(function (root) {
  ...
  // Indicazione sintetica dei tempi del catalogo semi, a partire dalla semina.
  function seedJourney(plant, lang) {
    ...
    return `<section class="seed-journey" aria-label="${ro ? "De la sămânță la recoltă" : "Dal seme alla raccolta"}">
      ...
      <p class="seed-journey-time"><small>${ro ? "Aproximativ" : "Circa"}</small><strong>${escapeHtml(plant.gg)}</strong><span>${ro ? "zile" : "giorni"}</span></p></div>
    </section>`;
  }
